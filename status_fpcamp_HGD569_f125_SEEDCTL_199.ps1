# status_fpcamp_HGD569_f125_SEEDCTL_199.ps1 -- READ-ONLY status probe for the
# R-SEED control campaign (2-type, donor-enriched elites, feed 125).  Emits a
# machine-parsable block; starts nothing.
#   ssh USER@HOST_199 "powershell -NoProfile -ExecutionPolicy Bypass -File C:\Users\USER\lpopt_work\kit_frontier\status_fpcamp_HGD569_f125_SEEDCTL_199.ps1"
$ErrorActionPreference = 'SilentlyContinue'
$k   = 'C:\Users\USER\lpopt_work\kit_frontier'
$r   = Join-Path $k 'runs\fpcamp_minfr_hgd569_f125_seedctl'
$log = Join-Path $k 'fpcamp_minfr_hgd569_f125_seedctl_out.log'
$rcf = Join-Path $k 'fpcamp_minfr_hgd569_f125_seedctl_rc.txt'
...
if (Test-Path $log) {
  ...
  Write-Output 'WAVES>>'
  # confirm the elite pool actually drew on the donor-enriched store (not a
  # silent fallback to an empty/backfill pool).
  Write-Output '<<SEED'
  Select-String -Path $log -Pattern 'elite|store_elite' | Select-Object -First 4 | ForEach-Object { $_.Line }
  Write-Output 'SEED>>'
  Write-Output '<<FAILS'
