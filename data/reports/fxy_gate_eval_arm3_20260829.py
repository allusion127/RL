"""ARM 3 -- copy of ``fxy_gate_eval_arm2c_20260829.py`` (itself a copy of the
untouched arm-2 harness ``fxy_gate_eval_arm2_20260829.py``) with ONLY ``CAND``,
``OUT`` and this header changed, exactly as prereg Amendment **C.5** registers.

BINDING adjudication of arm 3 (``--fxy-direct``, ``data/models/20260829_194532``)
against Amendment **C.4** -- G2' MAE < 0.0767, G3' rho-bar > 0.7263 (AND), on the
FIXED serve path (``featurize.serve_provenance``, C.3) with the per-cell
calibration artifacts REFIT on that same fixed path on BOTH sides.

The bar constants ``G2P_BAR`` / ``G3P_BAR`` are NOT touched (C.5).

Original arm-2 header follows.
...
nothing, edits no deck.
    ...
    python data/reports/fxy_gate_eval_arm3_20260829.py
"""
...
CAND = ROOT / "data/models/20260829_194532"
PREV = ROOT / "data/models/s1i"
STORE = ROOT / "data/store"
SPLIT = ROOT / "data/splits/S1j.json"
OUT = ROOT / "data/reports/fxy_gate_eval_arm3_20260829.json"
