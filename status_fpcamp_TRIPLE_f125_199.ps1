# status_fpcamp_TRIPLE_f125_199.ps1 -- READ-ONLY status probe for the first
# 3-fresh-type (graded) campaign at feed 125.  Emits a machine-parsable block;
# starts nothing.
#   ssh USER@HOST_199 "powershell -NoProfile -ExecutionPolicy Bypass -File C:\Users\USER\lpopt_work\kit_frontier\status_fpcamp_TRIPLE_f125_199.ps1"
$ErrorActionPreference = 'SilentlyContinue'
$k   = 'C:\Users\USER\lpopt_work\kit_frontier'
$r   = Join-Path $k 'runs\fpcamp_minfr_triple_f125'
$log = Join-Path $k 'fpcamp_minfr_triple_f125_out.log'
$rcf = Join-Path $k 'fpcamp_minfr_triple_f125_rc.txt'
...
if (Test-Path $log) {
  ...
  Write-Output 'WAVES>>'
  # 3-type specific: confirm the elite donor seeding fired and that the pool is
  # reaching the third fresh type at all.  A campaign that silently lost either
  # is not a null result, it is a broken run.
  Write-Output '<<SEED'
