...
> 3. **AI가 실측으로 확정·발견한 규칙**(§4) — 71,517~76,693행 MASTER 검증 노심과 사전등록 통제실험에서 **채굴(mined)** 된 규칙. 이 문서의 핵심. **§4.12는 2026-08-29 목적축 전환(`F_r` → `F_xy`) 이후 확정된 규칙**이고, **§4.13은 규칙으로 승격될 뻔했던 하네스 결함**이다.
>
...
| `data/reports/fxy_switch_design_20260829.md` | **목적축 전환 설계서** — `F_xy` 정의·`F_r` 관계·게이트 교차표(§4.12 M) |
| `data/reports/intervention_wave_r1_results_20260830.md` (+Amendment 1·2) | **개입 웨이브 r1** — 전달계수 분기·기술자 결손·누설매개 재판정(§4.12 N·O·R·S) |
| `data/reports/pinbu_wave_{fxyera,minfxy}_r1_results_20260830.md` | `F_xy` 결정성·핀 feed 편향·**첫 납품 가능 노심**(§4.12 P·Q·T·U·V) |
| `data/reports/hgd569_degeneracy_memo_20260830.md` | **규칙이 아니라 결함이었던 것**(§4.13) |
...
| `f_r` | 반경방향 첨두 (F_ΔH 계열) | – | 인허가 한계 **1.55** (`LICENSING_FR_LIMIT`); `flat_power`에서는 안전 게이트 **1.70**. **2026-08-29부터 목적이 아니라 제약** | `lpopt/search/delivery.py:43`, `config.py` `flatpower_fr_limit=1.7` |
| `f_q` | 3-D 출력 첨두 | – | **2.41** | `lpopt/config.py:465` |
| **`f_xy`** | **핀 평면 첨두** (MASTER `MAS_OUT`의 `FXYP`, 평형 최종 사이클 전 감손 스텝 최댓값) | – | **한계 1.65 · 2026-08-29부터 최적화 목적함수**(`min_fxy`). `F_r ≤ F_xy ≤ F_q` (192/192 실측) | `lpopt/data/fxy.py`, `config.py` `f_xy_limit=1.65` |
| `node_peak` | 집합체(노드) 출력 최댓값 — **`F_xy`가 아니다**(상관 0.735–0.854, 08-29 정정) | – | 목적함수(1차항), 하드 한계 없음 | `lpopt/data/flatness.py` |
...
#### (A) 누설-중재(leakage arbitration) — SETTLED (부호만; 크기는 §4.12 (S)에서 정정)
...
### 4.12 `F_xy` 시대에 확정·발견된 규칙 (2026-08-29 ~ 08-31)

2026-08-29 사용자 확정으로 최적화 목적이 **`min(max F_xy)`**(한계 1.65)로 바뀌었다. `F_r`은
제약으로 남는다. 축이 바뀌자 **기존 규칙 중 무엇이 축을 건너 이식되고 무엇이 안 되는지**가
바로 시험대에 올랐고, 그 답이 이 절이다.

#### (M) `F_xy ≈ 1.07·F_r` — 그러나 **게이트는 대체되지 않는다**
`fxy_switch_design_20260829.md`(2026-08-29, HOST_199 `MAS_OUT` 192 노심 + 로컬 curated 188 +
regen 690). 신뢰도 **높음(실측)**.

| 관계 | 값 |
|---|---|
| `F_xy/F_r` (풀드 192) | **1.0694 ± 0.0181** (p05 1.0351 / p95 1.0934) |
| `F_r ≤ F_xy ≤ F_q` | **192/192 성립** (`F_q/F_xy` = 1.1586 ± 0.0218) |
| 풀드 선형 | `F_xy = 1.1221·F_r − 0.0831` (r 0.9505, 잔차 sd **0.0293**) |
| 셀별 | `E1_E2` 1.1264·F_r − 0.0826 (sd 0.0315) · `N1_N2` 1.1416·F_r − 0.1209 (sd 0.0253) · regen `T6_T4` 1.0453·F_r + 0.0266 (sd 0.0320) |
| 로컬 curated / regen 비 | 1.0619 ± 0.0145 / 1.0620 ± 0.0201 |

**그런데 게이트 교차표가 대체 불가를 말한다.**

| 셀 | n | `F_r ≤ 1.55` 통과 | `F_xy ≤ 1.65` 통과 | **F_r 통과·F_xy 탈락** |
|---|---:|---:|---:|---:|
| `E1_E2`/f109 | 94 | 52 | 39 | **18 (52의 34.6%)** |
| `N1_N2`/f113 | 98 | 41 | 49 | **0** |
| regen `T6_T4`/f121 | 690 | 213 | 230 | **41** |

> **규칙(확정)**: **`F_r` 게이트로 `F_xy` 게이트를 대신할 수 없다 — 대체 실패는 셀 의존적이다.**
> 안전망으로 쓰려면 `F_r ≤ 1.509`(p95 비 1.0934 기준)까지 내려야 하고, 그것도 부분적이다.
> 부수 결과: **`F_xy ≤ 1.65`는 `F_q ≈ 1.91`을 함의**하므로 `F_q ≤ 2.41`보다 **훨씬 빡빡한
> 게이트**다.

**챔피언 역전도 실측됐다.** `T6_T4`/f121 690 노심에서 `F_r` 기록 코어(1.4797 @ 623.6 EFPD)의
`F_xy`는 1.5829인데, 개체군 `F_xy` 최소는 **다른 코어**(1.5491, F_r 1.5018)다. **목적함수를
바꾸면 순위가 재배열된다.**

#### (N) 전달계수는 무브 클래스에 의존한다 — **비율 프록시는 Gd 무브에 부적격**
`intervention_wave_r1_results_20260830.md` H3(2026-08-30, +Amendment 2 08-31; 5셀 800체인,
수렴 791). 신뢰도 **높음(사전등록 개입 실험)**.

같은 개입이 `F_r`과 `F_xy`를 **같은 비율로** 움직이지 않는다. 전달계수 `ΔF_xy : ΔF_r`가
무브 계열에 따라 두 배 이상 갈린다.

| 무브 계열 | 전달계수 |
|---|---|
| 농축·반경(`fresh_relocate` in/out, signed `batch_swap`) | **1.23 – 1.42** (v2: in 1.35 / out 1.42, signed batch_swap 1.76 / 1.22) |
| Gd·격자(`batch_swap\|neutral`, `batch_flip\|neutral`) | **0.55 – 0.73** |

H3의 부호 일치는 r1에서 4/5, **v2 재실행 후 5/5**다. 불일치 사례 하나가 진단적이다 —
`E1_E2/f109`의 `batch_flip|neutral`에서 `d_F_xy` **−0.0027 (p = 0.041)** 인데 `d_F_r`은
+0.0100 (p = 0.263)으로 **부호가 반대**다.

> **규칙(확정)**: **`F_xy ≈ a·F_r` 비율 프록시는 농축·반경 무브에서만 유효하고 Gd·격자
> 무브에서는 계수가 절반으로 떨어진다.** 이것이 상수 비율 프록시가 아니라 **헤드**가 필요한
> 물리적 이유다(→ [02](02_model_methodology.md) §6.8).

#### (O) Gd/격자 기술자 결손 — 현 피처 집합이 못 보는 자유도가 있다
같은 개입 웨이브 H4. 신뢰도 **높음**.

축퇴 셀에서 `batch_swap|neutral` vs `rewire_swap|neutral`의 `d_F_xy` 차이:

| 셀 | Δ | 부호 | p |
|---|---:|---|---|
| `E1_E2`/f109 | **+0.0712** | 20/20 | **1.9e-6** |
| `E1_E2`/f121 | **+0.0689** | 17/20 | **0.0026** |
| `N1_N2`/f113 | +0.0071 | 12/20 | 0.503 |

**`d_fresh_enr_r_center ≡ 0`인 개입이 `F_xy`를 0.07 움직인다.** 부모 `F_xy`가 1.53–1.83이고
한계가 1.65이므로 이것은 **정책적으로 큰 값**이다.

> **규칙(확정)**: **현 기술자 집합은 Gd·격자 축의 자유도를 담지 못한다.** 반응도 기술자만으로는
> 이 이동을 예측할 수 없다. 다음 피처 라운드의 1순위 후보다.

#### (P) `F_xy`는 재시작에 대해 **완전 결정론적**이다
`pinbu_wave_fxyera_r1_results_20260830.md` M2(40체인) + `pinbu_wave_minfxy_r1_results_20260830.md`
M2(20체인). 신뢰도 **높음(실측)**.

**`|ΔF_xy| = 0.000000` (60/60)** — `F_xya`도 동일. 스칼라 6축(`f_r`/`cyclen`/`cbc`/`f_q`/
`ao`/`max_assy_bu`)도 두 번째 웨이브에서 전부 **0.000000**이다.

> **규칙(운영)**: **`F_xy` 마진에 잡음 여유를 둘 필요가 없다.** `F_r`과 같은 등급의
> 재현성이므로 0.001 차이도 실신호로 읽는다.

#### (Q) 핀 헤드의 편향은 **feed에 따라 부호가 갈린다**
같은 두 웨이브 M4/M5. 신뢰도 **높음(복제됨)**.

| feed | n | bias | 판정 |
|---|---:|---:|---|
| 109 | 7 | **−1.416** [−3.48, +0.64] | **과소예측** — 실측 span 80.7–97.2, **0/5가 ≤80** |
| 117 | 3 | +1.153 | — |
| **121** | 13 | **+3.230** [+2.39, +4.07] (음수 잔차 **0개**) | **과대예측** — 실측 64.5–67.8, 5/5 PASS |
| 125 | 16 | +1.046 | 9/10 |

`pinbu_wave_minfxy_r1`이 feed 121에서 **+3.420 [+3.23, +3.61]** 로 이 관찰을 복제했다.
전체 bias +1.215 / MAE 2.193이라 재보정 트리거(|bias| ≥ 2.0)는 발동하지 않는다 — **풀링이
부호가 반대인 두 모집단을 상쇄**하기 때문이다.

> **규칙(운영)**: **단일 예측 게이트 `78 = 80 − 2.0`은 feed 109에서 부족하고 feed 121에서
> 과잉이다.** feed-계층화 마진이 필요하다. 실측 사례: 예측 75.73 → 실측 **80.737**(78 게이트
> 통과 후 80 위반).

보정 곡선도 갱신됐다 — `meas = 1.0474 × pred − 5.373`(n=20, r **0.974**; 2026-08-20의
1.0320 × pred − 1.953, n=21, r 0.933 대비 기울기는 1.5% 이내, 절편만 3.4 GWd/tU 하향).

#### (R) `batch_swap` 일반성 — **최종 부정**
`intervention_wave_r1` H2 + Amendment 2. 신뢰도 **높음(3 웨이브 누적)**.

HGD569 셀의 20/20 축퇴가 하네스 결함(§4.13)이었음이 밝혀져 **판정이 가능해졌다**. 부호는
**2/2 셀에서 성립**한다(T6T4 −0.0179, HGD569 −0.0177, pooled **−0.0178 [−0.037, +0.001]**),
그러나 **유의하지 않고**(p = 0.256) **크기가 세 웨이브에 걸쳐 단조 감쇠**한다.

| 웨이브 | out−in `d_F_r` |
|---|---:|
| batchswap wave (2026-08-15) | **−0.1442** [−0.1888, −0.1013] |
| (초기 추정) | −0.1711 |
| 개입 웨이브 r1 v2 (2026-08-31) | **−0.0178** |

> **규칙(확정)**: **`batch_swap`의 크기는 정책 피처로 전역 사용할 수 없다.** 08-15의
> "618 분지 특이성"(§4.10 C)에 이어 **크기 재현 실패**가 더해졌다. 부호만 남는다.

#### (S) 누설-중재는 **cell-conditional**이며 08-15의 크기는 과대추정이었다
같은 개입 웨이브 H1/H1b. 신뢰도 **높음(사전등록 개입, 5셀)**.

- **H1 (부호)**: `fresh_relocate` out−in `d_cyclen` < 0이 **5/5 셀에서 성립**하고 pooled는
  **−1.1649, sign 62/195, p = 4.0e-7**로 강하다. 그러나 셀별 유의는 **3/5**뿐이다
  (E1E2_f109 8.4e-6 · N1N2_f113 0.0095 · E1E2_f121 0.038; HGD569 0.243, T6T4 **0.636**).
- **H1b (크기)**: 08-15의 원 셀 `T6_T4`/f121에서 **재현 실패**했다.

| | 2026-08-15 어블레이션 | 2026-08-30 개입 r1 |
|---|---|---|
| `d_cyclen` (out−in) | **−1.7415** [−2.1336, −1.3342] | **−0.5171** [−1.020, −0.037] |
| 부호검정 | 0/10, p = 0.002 | **18/40, p = 0.636** |
| dose 기울기 | −8.37 EFPD/unit | **−1.031** [−3.88, +1.68] |

CI가 **비중첩**이다.

> **규칙(정정)**: **부호는 유지되고(§4.10 A는 살아 있다) 크기는 3.37배 과대추정으로 철회된다.**
> 등록된 인용 규약 "부호와 자릿수로만"이 여기서 다시 정당화됐다.
> dose 기울기는 셀 간 **29배** 산포한다(E1E2_f109 −30.285 vs T6T4 −1.031).
> burn-state 층화도 크다 — `once` −0.406 vs `twice_plus` **−4.190**(10–13배)이나
> `twice_plus` 79행이 전부 f109/f113이라 **burn-state × feed 교락이 미해소**다.

#### (T) `F_xy` 축에서 셀내 순위는 **모델-프리 비율 추정기가 헤드를 이긴다**
`pinbu_wave_minfxy_r1_results_20260830.md`(20 + 5기) + `minfxy_T6T4_f121_r1_results_20260830.md`.
신뢰도 **높음(실측, 두 문서 정합)**.
...
**bias 보정이 오히려 악화시킨다** — 잔차 bias가 group-A **−0.0075**, group-B **+0.0420**으로
부호가 반대다. 헤드의 셀내 랭킹 Spearman은 **−0.114 (p = 0.63)** 이고, `min_fxy` r1의
joint-clean 위에서는 **+0.016**(프록시 +0.179), exploit 슬롯에서는 **−0.383**이다.

> **규칙(운영)**: **`f_xy` 헤드는 셀 *간* 레벨용이고, 셀 *내* 순위에는 아직 비율 추정기를
> 병기해야 한다.** 단, (N)이 말하듯 **비율 추정기는 Gd 무브에서 무너진다** — 두 도구의
> 유효 영역이 서로 보완적이며 어느 쪽도 단독으로 충분하지 않다.

#### (U) `F_r` 기록 코어는 `F_xy` 축에서 거의 그대로 좋다 — 그래서 이득이 작다
`pinbu_wave_minfxy_r1_results_20260830.md` M4. 신뢰도 **높음(실측 5기)**.

| record | 캠페인 | `F_r` | **측정 `F_xy`** | `F_xy/F_r` | 실측 핀 |
|---|---|---:|---:|---:|---:|
| `4d70ab6f` | batchswap_enum_T6T4 | **1.4605** | **1.5402** | 1.0546 | 63.669 |
| `0a83b654` | batchswap_enum_T6T4 | 1.4612 | 1.5471 | 1.0588 | 63.878 |
| `eaf914e5` | batchswap_enum_T6T4 | 1.4648 | 1.5480 | 1.0568 | 63.470 |
| `456199b3` | batchswap_enum_T6T4 | 1.4616 | 1.5498 | 1.0603 | 63.493 |
| `188c9a33` | fpcamp_minfr_T6T4_r8 | 1.4749 | 1.5684 | 1.0634 | 63.665 |

`F_r` 시대 코어 4기가 `min_fxy` r1 top-20 안에서 **3–4위**에 든다.

> **규칙(해석)**: **`F_r` 최적화는 `F_xy`에 대해 헛되지 않았다 — 대부분의 이득이 이식된다.**
> 그 귀결로 `min_fxy` r1의 실질 이득은 **0.0080(−0.52%)** 에 불과하다. 그리고 `4d70ab6f`는
> `bf3a70b2`를 **`F_r`·`F_q`·CBC에서 압도**하고 `F_xy` 0.008과 cyclen 4.1 EFPD만 양보하므로,
> **납품 후보 선택은 단일 스칼라가 아니라 인허가 우선순위의 문제**다.

#### (V) 첫 납품 가능 노심에서 가장 타이트한 축은 **`F_r`** 이다
같은 문서 §6. 신뢰도 **높음(전 축 실측)**.

`bf3a70b2`의 축별 마진: **`F_r` 0.064** < `F_xy` 0.118 < `F_q` 0.557 < CBC 262.6 ppm <
\|AO\| 0.2756 < **핀 16.24 GWd/tU**.

핀 상관도 새로 측정됐다 — 집합체 연소도와 **r = +0.752**(핀/집합체최대 비 1.1412–1.1549),
`F_xy`와 **r = −0.606**, `F_r`과 −0.375. 즉 **평탄한 노심이 오히려 핀이 약간 뜨겁다**
(셀 내부 연소도 효과).

> **규칙(운영)**: `T6_T4`/f121 셀에서 핀은 **더 이상 결박축이 아니다**(마진 16 GWd/tU).
> 08-20의 "핀이 결박축"(§4.10 I)은 **저feed·고농축 셀에 한정된 규칙**으로 좁혀진다.

#### (W) 3종 계단화의 `F_r`-only 라인은 **붕소 벽에서 닫혔다**
`tripletype_f125_r2_results_20260829.md`(60/60 콜, 수렴 58). 신뢰도 **높음(NULL 사전등록)**.

- `F_r ≤ 1.55` 도달 **0/58**, 두 라운드 pooled **0/107**.
- **CBC-clean 프런티어는 1.5999에서 정지**하는데 raw 프런티어는 1.5864까지 간다 — 그 사이
  **8계단이 전부 CBC 위반**이다. pooled로 `F_r < 1.5993`인 17기 중 CBC-clean **0기**이고
  그 17기의 최소 CBC가 **1602.58 ppm**이다.
- **2배 예산 · 강화 엘리트(r1 최상위 보드가 부모) · 셀내 챔피언(`s1i`)으로도 클린 프런티어가
  +0.0006**(노이즈 이하) 움직였다. 웨이브별 클린 최선이 1.60 근방에서 **평탄하게 배회**한다.

> **규칙(확정)**: **3종 계단화에서 `F_r`을 막는 것은 `F_r`이 아니라 붕소(CBC)다.**
> 08-17의 "병목은 붕소"(장부 #29)가 3종 축에서 재확인됐다. 등록된 처분: **이 셀의 라운드 3은
> 금지**, 다음 지렛대는 같은 3종 알파벳의 반경 계단화가 아니라 **연료 타입 정의 자체**
> (축방향 zoning / 블랭킷 / Gd 로딩). r1이 "가장 유용한 물리"라 불렀던 mid-type dose-response는
> **부호가 뒤집혀 재현 실패**했다(`r(mid-type, F_r)` −0.417 → +0.282,
> `r(mid-type, CBC)` +0.638 → −0.581) → **격하**.

### 4.13 규칙이 아니라 결함이었던 것 — HGD569 축퇴 (2026-08-30 ~ 31)

**이 절이 여기 있는 이유**: 개입 웨이브 r1이 처음 보고한 두 "물리"가 실제로는 하네스 결함의
산물이었고, 규칙집이 그것을 규칙으로 승격시킬 뻔했다. 채굴된 규칙의 **세 관문**(§4.0)에
"라벨이 실제로 그 노심의 것인가"가 암묵적으로만 있었음을 드러낸 사례다.

**보고됐던 두 관측** — ① `HGD569_f125` 셀의 vs-parent delta 전체가 **+34.71 ± 1.71 EFPD /
+0.098 F_r / +0.106 F_xy** 이동, ② `batch_swap` **20/20 쌍이 4축 비트 동일 + power map
max\|Δ\| = 0**(완전 축퇴).

**실제 원인** (`hgd569_degeneracy_memo_20260830.md`): `WaveVerifier`를 `resolver=` 없이 만들면
alias 맵이 빈 resolver가 fallback으로 생성되고, `alias_pattern()`이 `type_to_alias`가 비면
**패턴을 그대로 반환**한다 → `%LPD_SHF`에 존재하지 않는 13자 batch id → **MASTER가 경고 없이
앞 2자 `P6`로 흡수** → 노심 전체가 단일 조성 `FA_P6`로 계산. alias 비교가 크기까지 설명한다:
`P6`의 `u_avg` **5.8812** / `gd_wt` 6.0 vs 의도한 S3 5.7861/6.0 + S5 5.6023/**10.0**
(설계 `e_core` 5.6944 → 실현 **5.8812 단일**) → 반응도 상승 = 주기 연장, **자릿수 정합**.

**v2 재실행이 되돌린 것:**

| 항목 | r1 (결함) | v2 (재실행 160체인) |
|---|---|---|
| 중립 대조 오프셋 | **+35.0533 EFPD** | **+0.0829 EFPD** |
| `F_r` / `F_xy` / `node_peak` 오프셋 | +0.0982 / +0.1056 / +0.0493 | +0.0262 / +0.0249 / +0.0239 |
| `batch_swap` 4축 비트 동일 쌍 | **20/20** | **0/19** |
| `s1i` 블라인드 HGD569 `d_cyclen` MAE | 35.682 EFPD | **0.992 EFPD** |
| 셀 상태 | "탈동조 셀" | **4축 정합 셀**(`d_node_peak` −0.0312, p = 0.0288) |

**규칙집에 남는 교훈 셋.**
1. **축퇴는 물리가 아닐 수 있다.** 4축이 비트 동일한 쌍이 20/20이면 그것은 발견이 아니라
   **경보**다. 대조로 `T6T4`의 같은 무브는 2.84–4.61 만큼 움직였다.
2. **공통모드는 중립 대조군으로 잡는다.** `rewire_swap|neutral` 오프셋이 다른 셀에서
   +0.023 ~ +0.174인데 이 셀만 **+35.05**였다 — 이 진단이 `analyze`에 `neutral_control_offset`
   으로 상설화됐다.
3. **모델의 블라인드 예측이 결함의 탐지기 역할을 했다.** `s1i`가 이 셀에서만 `d_cyclen`을
   +35.68 EFPD 틀렸고, 다른 셀 전체에서는 +0.095였다. **서로게이트가 크게 틀리는 셀은 물리가
   낯선 곳이거나 라벨이 잘못된 곳이다** — 후자를 먼저 의심할 이유가 생겼다.
...
| M | `F_xy ≈ 1.07·F_r`이나 **게이트는 대체 불가** | 확정 | 비 1.0694 ± 0.0181; `E1_E2`/f109에서 F_r 통과 52 중 **18이 F_xy 탈락** |
| N | 전달계수가 무브 클래스에 의존 | 확정(개입) | 농축·반경 **1.23–1.42** vs Gd/격자 **0.55–0.73** → 비율 프록시는 Gd 무브에 부적격 |
| O | Gd/격자 기술자 결손 | 확정(개입) | `d_fresh_enr_r_center ≡ 0`인데 `F_xy` **+0.07** (20/20, p=1.9e-6) |
| P | `F_xy`는 재시작 결정론적 | 확정(실측) | **\|ΔF_xy\| = 0.000000, 60/60** → 마진에 잡음 여유 불필요 |
| Q | 핀 헤드 편향이 feed로 갈린다 | 확정(복제) | feed 121 **+3.42** [+3.23,+3.61] vs feed 109 **−5.0** → 단일 게이트 78 부적절 |
| R | `batch_swap` 크기는 일반화 불가 | 반증(3웨이브) | −0.1711 → −0.1442 → **−0.0178** 단조 감쇠, pooled CI가 0 포함 |
| S | 누설-중재는 cell-conditional, 크기는 과대추정 | 정정(개입) | 부호 5/5·유의 3/5; 원 셀 재현 −0.52 vs −1.74, **CI 비중첩 → 3.37배 철회** |
| T | 셀내 순위는 모델-프리 비율이 헤드를 이긴다 | 확정(실측) | MAE **0.008** vs 0.042 (5배), 헤드 셀내 ρ **−0.114** |
| U | `F_r` 기록 코어가 `F_xy`에서도 거의 그대로 좋다 | 확정(실측) | `4d70ab6f` F_xy **1.5402** → r1 실질 이득 **0.008(−0.52%)** |
| V | 첫 납품 노심의 최타이트 축은 `F_r` | 확정(전축 실측) | 마진 F_r **0.064** < F_xy 0.118 < … < 핀 **16.24** |
| W | 3종 `F_r`-only 라인은 **붕소 벽**에서 닫힘 | 확정(NULL) | 클린 1.5999 정지 / raw 1.5864까지 8계단 전부 CBC 위반; F_r<1.5993인 17기 중 clean **0** |
...
| `F_xy` | MASTER `MAS_OUT`의 `FXYP` — 핀 평면 첨두. 평형 최종 사이클의 전 감손 스텝 최댓값. 한계 1.65. **2026-08-29 이후의 목적축** |
| `is_deliverable` / `is_feasible_search` | 납품 술어(모든 게이트 축이 **측정**되고 한계 내) / 탐색 술어(`max_pin_burnup`·`f_xy` 결측은 통과) |
