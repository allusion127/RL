"""Ship the v2 corpus to 238, launch ``lpopt.policy.train_v2`` there, poll, pull.

    python train_policy_v2.py --ts policy_v2

Identical in shape to ``train_policy_v1.py`` — same ``lpopt.remote`` push/status/
pull, same heartbeat/rc/DONE markers, same GPU-1 pinning via ``lpopt_gpu1.inp``
so GPU 0 stays free for whatever else the box is running.  Three files go over
by ``ship_file`` because ``push`` does not carry ``data/policy`` or ``data/design``:
the corpus, the fuel table, and the BLIND policy-v1 baseline predictions.
"""
...
V1_BASELINE = "data/design/policy_v2_v1_baseline.csv"
...
HB=$!
CUDA_VISIBLE_DEVICES={gpu} {python} -m lpopt.policy.train_v2 {args} \
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--ts", default="policy_v2")
    ...
    print("=== ship corpus + blind v1 baseline ===", flush=True)
    for rel in (STEPS, "data/store/fuel_types.parquet", V1_BASELINE):
    ...
    train_args = (
        ...
        f"--cache data/policy/_feature_cache_v2.npz "
        f"--v1-baseline {V1_BASELINE} "
